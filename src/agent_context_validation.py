...
execution, EOT flow) to maintain proper alternation. See designs/DECISIONS.md
            ...
            if "arguments" not in func:
                ...
                if not isinstance(func["arguments"], str):
                    ...
                    try:
                        ...
                        errors.append("function arguments must be valid JSON")
    ...
    if errors:
        ...
        if in_progress:
            errors = [
                ...
                and "consecutive" not in e.lower()
                and "synthetic" not in e.lower()
            ]
        if errors:
...
def get_pending_tool_ids(messages: list[dict]) -> set[str]:
...
def validate_tool_resolution(messages: list[dict]) -> list[str]:
