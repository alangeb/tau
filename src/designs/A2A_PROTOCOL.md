...
| `tools` | list[str] | yes | Sorted list of available tool names |
...
| Field | Type | Description |
|-------|------|-------------|
| `type` | str | `"query"`, `"queued"`, `"stream_chunk"`, `"heartbeat"`, `"response"` |
...
3. Server processes query, emitting `stream_chunk` messages as tools execute
...
Chunks are emitted during query processing. Each chunk has a `type` field and
...
> **Implementation status**: `fork_start` and `fork_end` chunk types are
> documented for forward compatibility but are **not yet emitted** by
> `agent_tool_executor.py`. The `_emit_a2a_chunk()` function currently only
> emits `tool_call`, `tool_result`, `tool_error`, and `assistant` chunks.
> Clients should handle these types gracefully if they appear.

**No-op**: chunks are not emitted when there is no active A2A request
(`_current_a2a_request_id` is `None`).
...
| `TOOL_BLOCKED` | Tool blocked by filter |
| `FORK_START` | Fork/subagent start |
| `FORK_END` | Fork/subagent end |
...
| `COMPRESS_*` | Context compression events |
