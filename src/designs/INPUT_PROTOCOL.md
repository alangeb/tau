...
The `!` prefix executes shell commands via the bash tool:
...
/fork <task>   # Fork with inherited context
/subagent <task>  # Spawn isolated subagent
/ctx           # Display context
...
- **Command Dispatch**: `agent_commands.py` - `CommandManager`
- **Shell Execution**: `tools/bash.py` - bash tool
...
3. **Test all prefixes**: Ensure all 6 prefixes (`#`, `#!`, `!`, `+`, `/`, `#/`, `#+`) are tested
