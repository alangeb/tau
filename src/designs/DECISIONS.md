...
**See also**: [ARCHITECTURE.md](ARCHITECTURE.md) (module inventory), [CONTEXT.md](CONTEXT.md) (context management), [EOT.md](EOT.md) (end-of-turn contract), [INDEX.md](INDEX.md) (design index)
...
| 2.11 | **Two-tier boundary strategy** — steps 5–6 (tool_pruning, redact_blocks) respect 50% boundary; steps 7–8 (tool_pruning_full, redact_blocks_full) scan entire context | Graduated escalation: protect recent context first, then compress everything if needed |
...
| 4.20 | **`wiki` tool** (`tools/wiki.py`) — manage wiki: get/set wiki path in `tau.json` under `wiki.path`, check wiki status (exists, git status, file count, size), search wiki content by query, add entries (topic, type, query), retrieve entries by topic; stored path persists across sessions | Wiki infrastructure + content management |
...
| 9.5 | **Cached discovery** — `CommandRegistry` caches discovered commands; create new instance for invalidation | Performance with invalidation support |
...
| 10.5 | **Shared skill discovery** (`lib/skill_discovery.py`) — `SkillInfo` TypedDict, `discover_skills()` supports both folder-per-skill (`skills/name/SKILL.md`) and legacy flat-file (`skills/name.md`) formats; deduplication with folder-per-skill winning; `skill_name_from_path()` is canonical name extraction; `SkillInfoExtended` adds keywords/when/content; `discover_skills_extended()` parses extended metadata; `score_skill()` computes relevance via weighted word-set intersection; `suggest_skills()` ranks and returns top-N — used by `tools/skill.py`, `validate_skills.py`, `skill_finder.py`, `skills/tauskillmaintenance/skill_discover.py` | Single source of truth for skill path resolution |
...
| 23.2 | **`$HOME` over literal paths** — use `$HOME` or relative paths instead of `/home/user`; `alangeb` is the only acceptable personal reference and only in `$HOME` context, and even that should be avoided where possible | Path portability; eliminates user identity from codebase |
...
## Security Philosophy (2)

| # | Decision | Rationale |
|---|----------|-----------|
| 29.1 | **Bash access = explicit trust boundary** — Once `bash` tool is available with `shell=True`, no code-level sandbox can contain the agent. Dangerous command patterns (`DANGEROUS_PATTERNS`) serve as LLM nudges/reminders, not security gates. The REAL security boundary is the environment: containers, VMs, namespace isolation, read-only filesystems. Tau will NOT attempt code-level bash sandboxing. | Honest about capabilities; avoids false sense of security; focuses on what actually works (environmental isolation) |
| 29.2 | **File sandbox is best-effort for file tools only** — `tools/lib/sandbox.py` enforces path boundaries for `file_read`, `file_write`, `file_edit`, etc. This protects against LLM mistakes when bash is NOT available. When bash IS available, the sandbox is advisory only. Double-call confirmation is a UX pattern (gives the LLM a "pause and think" moment), not a security mechanism. | Useful guardrail for read-only/file-only tool configurations; clear that it is NOT a security boundary when bash is enabled |
