...
class TestDynamicCompressionThreshold(unittest.TestCase):
    ...
    def _make_agent(self, escalation_level=0, context_tokens=0, max_context_tokens=128000):
        ...
        agent._session.last_exact_context_tokens = context_tokens if context_tokens > 0 else None
        agent.context.estimate_tokens.return_value = context_tokens
    ...
    def test_level_0_no_pre_call_compression(self):
        ...
        agent = self._make_agent(escalation_level=0, context_tokens=100000, max_context_tokens=128000)
    ...
    def test_level_1_below_threshold_no_compression(self):
        ...
        agent = self._make_agent(escalation_level=1, context_tokens=60000, max_context_tokens=128000)
        # 60000/128000 = 46.9% < 65%
        compress_threshold = 0.65
        current_tokens = agent._session.last_exact_context_tokens or agent.context.estimate_tokens()
    ...
    def test_level_1_above_threshold_compression(self, mock_warning):
        ...
        agent = self._make_agent(escalation_level=1, context_tokens=90000, max_context_tokens=128000)
    ...
    def test_level_2_below_threshold_no_compression(self):
        ...
        agent = self._make_agent(escalation_level=2, context_tokens=30000, max_context_tokens=128000)
    ...
    def test_level_2_above_threshold_compression(self, mock_warning):
        ...
        agent = self._make_agent(escalation_level=2, context_tokens=50000, max_context_tokens=128000)
    ...
    def test_level_3_above_threshold_compression(self):
        ...
        agent = self._make_agent(escalation_level=3, context_tokens=50000, max_context_tokens=128000)
    ...
    def test_no_exact_context_tokens_falls_back_to_estimate_tokens(self):
        """When last_exact_context_tokens is None, falls back to estimate_tokens()."""
        agent = self._make_agent(escalation_level=1, context_tokens=0, max_context_tokens=128000)
        # last_exact_context_tokens is None, so should fall back to estimate_tokens()
        current_tokens = agent._session.last_exact_context_tokens or agent.context.estimate_tokens()
        self.assertEqual(current_tokens, 0)
        agent.context.estimate_tokens.assert_called_once()

    def test_exact_tokens_zero_falls_back_to_estimate_tokens(self):
        """When last_exact_context_tokens is 0, falls back to estimate_tokens()."""
        agent = self._make_agent(escalation_level=1, context_tokens=50000, max_context_tokens=128000)
