...
class TestRealAgentMetadata:
    ...
    def test_original_task_set_on_first_user_message(self, test_config):
        ...
        with patch.object(TauErgon, "invoke_with_tools_loop", return_value="ok"):
            agent = TauErgon(
                config=test_config,
                base_url="http://test:8000/v1",
                model="test-model",
            ...
            agent.invoke_with_tools("Investigate the build system")
            agent.invoke_with_tools("A follow-up question")
