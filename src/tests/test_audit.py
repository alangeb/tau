...
def test_parse_audit_file_basic(tmp_path: Path):
    ...
    f = tmp_path / "test.audit"
    f.write_text("[2026-06-23T17:24:08+00:00] USER nesting=0\n  | hello world\n")
...
def test_parse_audit_file_nested(tmp_path: Path):
    ...
    f = tmp_path / "test.audit"
    f.write_text(
        "[2026-06-23T17:24:08+00:00] FORK_START task=\"test\" nesting=0\n"
        "[2026-06-23T17:24:08+00:00] USER nesting=1\n"
        "  | nested message\n"
        "[2026-06-23T17:24:08+00:00] FORK_END duration_s=1.0 nesting=0\n"
    )
...
def test_parse_audit_file_old_format(tmp_path: Path):
    ...
    f = tmp_path / "test.audit"
    f.write_text("[2026-06-23T17:24:08+00:00] USER\n  | hello\n")
...
def test_parse_audit_file_with_fields(tmp_path: Path):
    ...
    f = tmp_path / "test.audit"
    f.write_text(
        "[2026-06-23T17:24:08+00:00] TOOL_CALL id=call_1 final_name=fetch fixes=none nesting=0\n"
        "  | final_args:\n"
...
def test_parse_audit_file_quoted_values(tmp_path: Path):
    ...
    f = tmp_path / "test.audit"
    f.write_text(
        "[2026-06-23T17:24:08+00:00] FORK_START task='test task with spaces' nesting=0\n"
    )
